...
$fx (thrdpool, uninit)
{
  ...
  thrd->tail = true;
  $call (blocking_queue, push, &this->mbr$ (cancel_queue), &thrd->list_entry);
      ...
      for (unsigned i = size; i < this->size; ++i)
        {
          ...
          thrd->tail = false;
          $call (blocking_queue, push, &this->mbr$ (cancel_queue),
                 &thrd->list_entry);
        }
      if (size == 0)
...
$fx (thrdpool, exec, void (*fn) (void *), void *arg)
{
  ...
  task->fn = fn;
  task->arg = arg;
  $call (blocking_queue, push, &this->mbr$ (exec_queue), &task->list_entry);
}
...
cancel (void *arg)
{
  ...
  for (;;)
    {
      auto node = $call (blocking_queue, shift, &this->mbr$ (cancel_queue));
  ...
  call$ (&this->mbr$ (cancel_queue), uninit);
  call$ (&this->mbr$ (exec_queue), uninit);
  free (this);
  pthread_exit (null);
}
...
$fx (thrdpool, init)
{
  $call (blocking_queue, init, &this->mbr$ (exec_queue));
  $call (blocking_queue, init, &this->mbr$ (cancel_queue));
